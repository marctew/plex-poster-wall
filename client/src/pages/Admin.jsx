  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-backdrop text-slate-200 min-h-screen">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 mb-6">
        ...
        <button onClick={async()=>{try{setLibs(await API.getLibraries()); setStatus('Loaded libraries.');}catch{setStatus('Failed to load libraries.');}}}
                className="mt-3 px-3 py-2 rounded bg-slate-800 hover:bg-slate-700">Reload Libraries</button>
      </div>

      {/* Libraries */}
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 mb-6">
        <div className="text-sm text-slate-400 mb-2">Libraries</div>
        {libs.length === 0 ? <div className="text-sm text-slate-500">Load libraries to choose sections.</div> :
      ...
      {/* TMDB */}
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 mb-6">
        <div className="text-sm text-slate-400 mb-2">TMDB</div>
        <label className="flex items-center gap-2 mb-2">
          <input type="checkbox" checked={!!cfg.show_tmdb} onChange={e=>setCfg({...cfg, show_tmdb:e.target.checked})}/>
        ...
        <input className="w-full bg-slate-800 rounded p-2"
               value={cfg.tmdb_api_key || ''} onChange={e=>setCfg({...cfg, tmdb_api_key:e.target.value})}
