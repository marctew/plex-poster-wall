...
export default function PosterCarousel({ items = [], dwell = 3500, cfg }) {
  ...
  const tmdb = useTmdb(current?.ratingKey);

  return (
    <div className="absolute inset-0 flex items-center justify-center">
      <AnimatePresence mode="wait">
        {current?.artUrl && (
          <motion.img key={current.ratingKey + '-art'} src={current.artUrl} alt=""
            className="absolute inset-0 w-full h-full object-cover"
            initial={{ opacity: 0 }} animate={{ opacity: backOpacity }} exit={{ opacity: 0 }}
            transition={{ duration: 0.6 }} style={{ filter: `blur(${blurPx}px)` }} />
        )}
      </AnimatePresence>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center max-w-[92vw]">
        ...
        {current && (
          <div className="mt-4 text-center px-6 max-w-[92vw]">
            ...
            {cfg?.show_synopsis && (
              <div className="mt-2 text-slate-200/85"
